...
#define PACKET_TYPE	ETH_P_LOOPBACK
...
void vdev_create_socket(struct vdev_info *dev)
{
	struct ifreq ifr;

	dev->sock = socket(AF_PACKET, SOCK_RAW, htons(PACKET_TYPE));//htons(ETH_P_ALL));
	assert(dev->sock != -1);
	...
	assert(ioctl(dev->sock, SIOCGIFFLAGS, (void*) &ifr) >= 0);
        /* Set the flags that bring the device up */
        ifr.ifr_flags |= ( IFF_UP | IFF_RUNNING );
        assert(ioctl(dev->sock, SIOCSIFFLAGS, (void*) &ifr) >= 0);
}

void vdev_send_packet(struct vdev_info *dev)
{
	...
	saddrll.sll_protocol = htons(PACKET_TYPE);
...
static void vdev_info_init(struct vdev_info* dev, unsigned long long features)
{
	...
	eh->ether_type = htons(PACKET_TYPE);
			...
			bool delayed, int batch, int bufs)
{
	...
	for (;;) {
		...
		do {
			...
			/* Flush out completed bufs if any */
			while (virtqueue_get_buf(vq->vq, &len)) {
				struct ether_header *eh =(struct ether_header *)(dev->res_buf + HDR_LEN);
				int i;

				/* tun netdev is up and running, ignore the non-PACKET_TYPE packet */
				if (eh->ether_type != htons(PACKET_TYPE)) {
					++vq->completed;
					r = -1;
					break;
				}
				...
				for (i = ETHER_HDR_LEN; i < TEST_BUF_LEN; i++)
					if(dev->res_buf[i + HDR_LEN] != (char)i)
						printf("rx buf: %x != %d\n", dev->res_buf[i + HDR_LEN], i);
