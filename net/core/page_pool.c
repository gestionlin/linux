...
#define PAGE_POOL_PUT_CTX_CNT_BIAS	(-1U << 31)
static void page_pool_item_uninit(struct page_pool *pool)
{
	struct page_pool_item *items = pool->items;
	unsigned int unmapped = 0;
	unsigned int item_cnt, i;
	int val;

	if (!pool->dma_map || pool->mp_priv)
		return;

	item_cnt = pool->p.pool_size + PP_ALLOC_CACHE_SIZE +
		PAGE_POOL_MIN_INFLIGHT_ITEMS;
	item_cnt = roundup_pow_of_two(item_cnt);

	val = atomic_add_return_acquire(PAGE_POOL_PUT_CTX_CNT_BIAS,
					&pool->put_ctx_cnt);
	/* Wait for any pending put context to finish */
	while (val != PAGE_POOL_PUT_CTX_CNT_BIAS) {
		might_sleep();
		usleep_range(1, 10);
		val = atomic_read_acquire(&pool->put_ctx_cnt);
	}
	...
	WARN_ONCE(unmapped != page_pool_inflight(pool, false),
		  "page_pool(%u) unmapped pages(%u) != inflight pages(%d)\n",
		  pool->user.id, unmapped, page_pool_inflight(pool, false));

	pool->dma_map = false;
	atomic_sub_return_release(PAGE_POOL_PUT_CTX_CNT_BIAS,
				  &pool->put_ctx_cnt);
}
...
static bool page_pool_item_add(struct page_pool *pool, netmem_ref netmem)
{
	...
	if (unlikely(llist_empty(&pool->hold_items))) {
		if (unlikely(llist_empty(&pool->release_items))) {
...
static void page_pool_item_del(struct page_pool *pool, netmem_ref netmem)
{
	...
	item->pp_netmem = (netmem_ref)NULL;
...
{
	/* Wait for page_pool_item_uninit() to do the dma unmapping for
	 * inflight pages in page_pool_destroy(), which is an unlikely case.
	 */
	while(!atomic_inc_unless_negative(&pool->put_ctx_cnt))
		udelay(1);
}
