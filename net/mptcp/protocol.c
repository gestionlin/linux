					   ...
					   unsigned int *offset)
{
	...
	va = page_frag_alloc_prepare(pfrag, size, offset, sk->sk_allocation);
	if (likely(va))
		return va;
...
static int mptcp_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	while (msg_data_left(msg)) {
		...
		size = 32U;
		va = mptcp_page_frag_alloc_prepare(sk, pfrag, &size, &offset);
		if (!va)
			goto wait_for_memory;
		...
		if (!dfrag_collapsed) {
			...
			va = mptcp_page_frag_alloc_prepare(sk, pfrag, &size,
							   &offset);
			if (!va)
				goto wait_for_memory;
		 ...
		 * anyway
		 */
		poffset = dfrag->offset + dfrag->data_len;
