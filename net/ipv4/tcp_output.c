...
static int tcp_send_syn_data(struct sock *sk, struct sk_buff *syn)
{
	...
	if (space) {
		if (!page_frag_cache_refill(nc, min_t(size_t, space, PAGE_SIZE),
					    sk->sk_allocation))
			goto fallback;
	}
	...
	if (space) {
		space = min_t(size_t, space, page_frag_cache_remaining(nc));
		space = tcp_wmem_schedule(sk, space);
	}
	if (space) {
		void *va = page_frag_cache_virt(nc);
		...
		skb_fill_page_desc(syn_data, 0, page_frag_cache_page(nc),
				   page_frag_cache_offset(nc), space);
		page_frag_cache_commit(nc, space);
