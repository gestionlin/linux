...
void free_unref_page(struct page *page, unsigned int order)
{
	...
	zone = page_zone(page);
	pfn = page_to_pfn(page);
	if (!free_unref_pcp_page(zone, page, pfn, order))
...
retry_this_zone:
	...
	if (page_array) {
		...
		for (i = nr_populated; i < nr_pages; i++) {
			if (!page_array[i])
				nr_account++;
		}
	...
	INIT_LIST_HEAD(&pcp_list);
	nr_account = rmqueue_pcplist_bulk(zone, nr_account, ac.migratetype,
					  alloc_flags, &pcp_list);
	if (!nr_account)
		goto failed;

	while(!list_empty(&pcp_list)) {
		page = list_first_entry(&pcp_list, struct page, lru);

		/* Skip existing pages */
		while (page_array && page_array[nr_populated]) {
			nr_populated++;
			continue;
		}
		...
		int write, void *buffer, size_t *length, loff_t *ppos)
{
	...
	ret = percpu_pagelist_high_fraction_set(percpu_pagelist_high_fraction);
	if (ret)
		goto err;
